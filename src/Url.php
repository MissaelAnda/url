...
use MissaelAnda\Url\Traits\Macroable;
use Psr\Http\Message\UriInterface;
use Spatie\Url\Exceptions\InvalidArgumentException;
use Stringable;
...
class Url implements UriInterface, Stringable
{
    ...
    public const VALID_SCHEMES = ['http', 'https', 'mailto', 'tel', 'ws', 'wss'];
    ...
    public static function parse(string $url): static
    {
        if (!$parts = parse_url($url)) {
            throw InvalidArgumentException::invalidUrl($url);
        }
        ...
        $url->path = $parts['path'] ?? '/';
        $url->query = QueryParameterBag::parse($parts['query'] ?? '');
    ...
    public function getQuery(): QueryParameterBag
    {
        return $this->query;
    }
    ...
    {
        $this->query->setUrlencoded($urlencoded);
    ...
    {
        $this->query->setEnumerated($enumerated);
    ...
    public function withQuery($query): static
    {
        ...
        $url->query = QueryParameterBag::parse($query);
